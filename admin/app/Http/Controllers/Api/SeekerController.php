...
//        array_push($jobs,$x);
    ...
    public function searchJob(Request $request){
        ...
        elseif ($data['value'] == null && (count($data['experience']) == 0 || count($data['qualification']) == 0 || count($data['job_location']) == 0 || count($data['job_type']) == 0 || count($data['specialization']) == 0 || count($data['job_by_roles']) == 0 || count($data['area_of_sector']) == 0)){
            ...
            $location_job = LocationModel::where('location_name','like','%'.$data["value"].'%')->get();
            $job_role = JobByRolesModel::where('job_by_role','like','%'.$data["value"].'%')->get();
            ...
            elseif ($job_role->count() > 0){
                $job_role_ids = [];
                foreach ($job_role as $key_role => $val_role){
                    array_push($job_role_ids,$val_role->id);
                }
                $tem_jobs = JobsModel::GetSearchedJobsWithRole($data['value'],$job_role_ids ,$data['experience'],$data['qualification'],$data['job_location'],$data['job_type'],$data['specialization'],$data['job_by_roles'],$data['area_of_sector'])->get();
            }
            elseif ($location_job->count() > 0){
                $locaton_ids = [];
                foreach ($location_job as $key_loca => $val_loca){
                    array_push($locaton_ids,$val_loca->id);
                }
                $tem_jobs = JobsModel::GetSearchedJobsWithLoca($data['value'],$locaton_ids ,$data['experience'],$data['qualification'],$data['job_location'],$data['job_type'],$data['specialization'],$data['job_by_roles'],$data['area_of_sector'])->get();
            }
            else{
