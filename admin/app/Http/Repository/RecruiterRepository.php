...
use Carbon\Carbon; //отвечает за время
        ...
        try {
            ...
            if ($data->hasFile('org_logo')) {
                ...
                if($check == null){
                    ...
                    $temp_data['org_logo'] = $check->org_logo;
                }
...
//              $model->insert($temp_data);
            ...
            $returndata['recruiter_profile']['org_logo'] = asset('public/storage/' . $logo);
...
//$data-полученный массив данных Request RecruiterRepository 
...
//                dd($x['profile']['seeker_area_of_sector']);
            ...
            if($resume)
            {
                $x['profile']['resume'] = asset('public/storage/' . $resume);
            }
            else
            {
                $x['profile']['resume'] = '';
            ...
            $x['profile']['avtar'] = asset('public/storage/' . $avtar);
